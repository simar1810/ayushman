...
const Subscription = () => {
  ...
  const fetchData = async () => {
    try {
      const data = await getAllSubscriptionsForAdmin(currentPage, token);
      ...
      console.log("getAllSubscriptionsForAdmin data => ", data);
      setsubscription(data?.data || []);
      setsubscriptioncount(data?.totalCount || 0);
    } catch (error) {
      toast.error("Error While fetching Subscriptions!");
      console.error("Error fetching subscription:", error);
    }
  ...
  return (
    <AdminDashboard>
      <div className="text-center text-sm mb-3">
        Total Subscription &nbsp;-&nbsp; {subscriptioncount}
      </div>
      ...
      <div className="flex justify-center items-center gap-10 mt-5 text-sm ">
        ...
        <span className="text-black">{currentPage}</span>
        <button
          className={`px-3 py-1 bg-slate-900 rounded text-[12px] hover:scale-105 text-[yellow] active:scale-95 transition-transform ${currentPage * 10 >= subscriptioncount
            ? "opacity-50 cursor-not-allowed"
            : "cursor-pointer"
            }`}
          onClick={() => handlePageChange(currentPage + 1)}
          disabled={currentPage * 10 >= subscriptioncount}
        >
          Next
        </button>
      </div>
          ...
          <tbody>
            {subscription?.map((subs, index) => (
              ...
              >
                ...
                >
                  {/* {subs?.coach?.name} */}
                  {subs?.coachId?.name || 'N/A'}
                </th>

                <td className="px-4 py-4">
                  {subs?.coachId?.coachId || 'N/A'}
                </td>

                <td className="px-4 py-4">
                  {subs?.coachId?.email || 'N/A'}
                </td>
                ...
                {/* <td className="px-4 py-4">
                  {subs?.razorpay_order_id || 'N/A'}
                </td> */}

                <td className="px-4 py-4">
                  {displayPlan(subs?.planCode) || 'N/A'}
                  {/* {subs?.isResolved ? (
                    <p
                      onClick={() => showModal2(subs)}
                      className="bg-green-500 w-[70px] text-center py-1 cursor-pointer px-2 rounded hover:opacity-90 text-white text-[12px]"
                    >
                      Resolved
                    </p>
                  ) : (
                    <button
                      className="bg-red-500 py-1 w-[70px] text-center px-2 rounded hover:opacity-95 text-white text-[12px]"
                      onClick={() => showModal(subs)}
                    >
                      Resolve
                    </button>
                  )} */}
      ...
      <div
        className={`fixed inset-0 flex items-center justify-center z-50 md:pl-[150px]  ${Add ? "" : "hidden"
          }`}
      >
            ...
            <div className=" mt-8">
              <div>
                <textarea
                  ...
                  id="message"
                  rows="6"
                  className="mt-2 md:w-[350px] block p-2.5 w-full text-sm text-gray-900 outline-none bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
              ...
              <div className="flex justify-end pt-5">
                <button
                  onClick={() => Confirm()}
                  className="text-sm bg-lime-500 mr-2 hover:bg-lime-600 hover:scale-105 active:scale-95 transition-transform text-white font-semibold py-2 px-4 rounded cursor-pointer"
                >
                  Yes
                </button>
                ...
                <button
                  ...
                  className="bg-red-500 text-sm hover:bg-red-600 hover:scale-105 active:scale-95 transition-transform text-white font-semibold py-2 px-4 rounded mr-2"
                >
                  Cancel
                </button>
              </div>
      ...
      <div
        className={`fixed inset-0 flex items-center justify-center z-50 md:pl-[150px]  ${unresolve ? "" : "hidden"
          }`}
      >
        <div className="fixed top-0 right-0 bottom-0 left-0 bg-black opacity-60"></div>
        <div className="bg-white p-8 rounded-lg shadow-lg z-10">
          <div>
            <div className="font-bold">
              Are You Sure You want to Unresolve This ?
            </div>
            <div>
              {/* <input autoComplete="off" name="organisation" type="text" placeholder="Organisation" className="w-[300px] border border-gray-500 h-5 px-3 py-5 mt-2  focus:outline-[#8080f3] rounded-md" /> */}
            </div>
