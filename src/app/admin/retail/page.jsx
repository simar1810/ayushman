...
import { getCookie, getBrand, getallRetailController, updateRetailController, DeleteRetail } from "@/actions/auth";
...
const Retail = () => {
    ...
    const handleSubmit = (productId) => async (e) => {
        try {
            e.preventDefault();
            setIsEditOpen(false);
            ...
            await updateRetailController(formData, token, productId).then(data => {
    ...
    const showEditModal = (user) => {
        ...
        const items = typeof user?.productMrpList === "string"
            ? JSON?.parse(user?.productMrpList || "{}")
            : user?.productMrpList;
        const pairsArray = Object?.entries(items).map(([key, value]) => ({ key, value }));
        setPairs(pairsArray);
        setList(pairsArray)
    };
    ...
    const fetchData = async () => {
        try {
            const data = await getallRetailController(currentPage, debouncedSearchQuery, token);
            setretail(data?.data);
    ...
    return (
        <AdminDashboard>
            ...
            {retailcount === 0 && <div className="text-center text-[black] mt-10 text-lg font-bold">No Retail Found</div>}
            ...
            <div className="flex justify-center flex-wrap gap-8 p-2 mt-5 rounded">
                {retail && retail.map((retail, index) => (
                    <div key={index} className="w-[320px] bg-white shadow text-center rounded">
                        ...
                        <div className=" whitespace-pre-wrap p-4">{retail.productDescription}</div>
                        <div className=" p-4">
                            ...
                            <div className="relative overflow-x-auto sm:rounded-lg">
                                <table className="mx-auto w-[280px] text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 shadow-lg">
                                    ...
                                    <tbody>
                                        {retail?.productMrpList && Object.entries((typeof retail?.productMrpList == 'string')
                                            ? JSON?.parse(retail?.productMrpList)
                                            : retail?.productMrpList)?.map(([key, value]) => (
            ...
            <div className={`fixed inset-0 flex items-center justify-center z-50 md:pl-[150px] ${isEdit ? '' : 'hidden'} `}>
                ...
                <div className="bg-white p-8 rounded-lg shadow-lg z-10 max-h-[600px] overflow-y-auto" >
                    <div>
                        ...
                        <br />
                        <select id="countries" value={brandId} onChange={(e) => setBrandId(e.target.value)} className="mb-5 bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:ring-1 block w-[300px] p-2.5 outline-none  ">
                        ...
                        <textarea onChange={handleChange('productDescription')}
                        ...
                        {pairs.map((pair, index) => (
                            <div key={index} className="flex space-x-4 my-2">
                                ...
                                {index !== 0 && <button type="button"
                                    onClick={() => handleRemovePair(index, pair?.key)}
                        ...
                        <div className="flex justify-end mt-8">
                            <button
                                onClick={handleSubmit(currentuserID._id)}
                                className="text-sm bg-lime-600 mr-2 hover:bg-lime-700 hover:scale-105 active:scale-95 transition-transform text-white font-semibold py-2 px-4 rounded cursor-pointer">Update</button>
                            <button
                                onClick={(e) => {
                                    e.preventDefault();
                                    hideModel();
                                }}
                                className="bg-slate-800 text-sm hover:bg-slate-900 hover:scale-105 active:scale-95 transition-transform text-white font-semibold py-2 px-4 rounded mr-2">Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
